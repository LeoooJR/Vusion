...
from config import ConfigParser, Expression, ExpressionTemplate, Term, TreeToExpression
...
def valid_config():
    return {
        "caller": {
            ...
            "depth": {"extract": "DP"},
            "rrc": {
                "forward": {"extract": "AD[0]"},
                "reverse": {"extract": "AD[1]"},
                "total": {"extract": "AD[0] + AD[1]"},
            },
            "arc": {
                "forward": {"extract": "AD[0]"},
                "reverse": {"extract": "AD[1]"},
...
class TestDSLParser:
    ...
    def test_complex_expression(self, transformer: TreeToExpression):
        ...
        result = transformer.transform(
            ConfigParser.DSL_PARSER.parse("(AD[0] + AD[1]) * 2")
        )
        assert isinstance(result, Expression)
        assert result.operator == "*"
        assert len(result.terms) == 2
...
class TestExpressionTemplating:
    ...
    def test_expression_templating(self):
        ...
        term_expr = {
            "type": "term",
            "field": "DP",
            "metadata": {"index": 0, "header": None, "unit": None},
        }
        ...
        complex_expr = {
            ...
            "terms": [
                ...
                },
                {
                    "type": "term",
                    "field": "AD",
                    "metadata": {"index": 1, "header": None, "unit": None},
                },
            ],
        }
        ...
        term_with_metadata = {
            "type": "term",
            "field": "AF",
            "metadata": {"index": 0, "header": "format", "unit": "%"},
        }
